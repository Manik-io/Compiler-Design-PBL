function renderComparison(grammar, inputTokens) {
  ...
  tryRun("LL(1)", "Top-down", false, function() {
    ...
    return { name:"LL(1)", desc:"Top-down", build:t1-t0, parse:performance.now()-t1,
      accepted:pr.accepted, conflicts:ll1r.conflicts.length, states:null, steps:pr.steps.length };
  });
  ...
  tryRun("LR(0)", "Bottom-up (no lookahead)", true, function() {
    ...
    return { name:"LR(0)", desc:"Bottom-up (no lookahead)", build:t1-t0, parse:performance.now()-t1,
      accepted:pr.accepted, conflicts:r.conflicts.length, states:r.states.length, steps:pr.steps.length };
  });
  ...
  tryRun("SLR(1)", "LR(0) + FOLLOW sets", true, function() {
    ...
    var t1 = performance.now();
    var pr = parseLR(grammar, r.action, r.goto, inputTokens);
    return { name:"SLR(1)", desc:"LR(0) + FOLLOW sets", build:t1-t0, parse:performance.now()-t1,
      accepted:pr.accepted, conflicts:r.conflicts.length, states:r.states.length, steps:pr.steps.length };
  });
  ...
  tryRun("LALR(1)", "Merged LR(1) states", true, function() {
    ...
    var t1 = performance.now();
    var pr = parseLR(grammar, r.action, r.goto, inputTokens);
    return { name:"LALR(1)", desc:"Merged LR(1) states", build:t1-t0, parse:performance.now()-t1,
      accepted:pr.accepted, conflicts:r.conflicts.length, states:r.states.length, steps:pr.steps.length };
  });
  ...
  html += barSection("Build Time",   results, function(r) { return r.build; },     maxBuild, "#0891b2", function(r) { return r.build.toFixed(3) + "ms"; });
  html += barSection("Parse Steps",  results, function(r) { return r.steps; },     maxSteps, "#7c3aed", function(r) { return r.steps; });
  html += barSection("Conflicts",    results, function(r) { return r.conflicts; },  maxConf,  "#dc2626", function(r) { return r.conflicts; });

  html += '<div class="bar-section" style="overflow:auto;">';
  html += '<div class="bar-title">Parser Comparison</div>';
  ...
  html += powerRow("Left recursion", ["\u2717",            "\u2713",     "\u2713",     "\u2713"]);
  html += powerRow("Lookahead",      ["1 token",           "None",       "FOLLOW set", "Item-specific"]);
  html += powerRow("Power level",    ["Weak",              "Weakest",    "Medium",     "Strong"]);
  html += powerRow("Real-world use", ["Recursive descent", "Rare",       "Academic",   "Yacc/Bison"]);
